  ...
  return (
    ...
    >
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <Text style={[{ textAlign: item.role === "user" ? "right" : "left", marginLeft: item.role === "user" ? 80 : 0, width: Platform.OS === "web" ? 200 : 280 }, styles.mainChat]}>
            {item.content}
          </Text>
        )}
        inverted
...
function createStyle(colors) {
  ...
  bottom: 0,
  right: 0,
  width: "106%",       // instead of 106%
  paddingBottom: 10,   // smaller padding so input isn’t hidden
  height: "40%",
